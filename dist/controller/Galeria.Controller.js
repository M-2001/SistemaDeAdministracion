    ...
    if (req.files === undefined || req.files.foto === undefined) {
        ...
        if (extFile.indexOf(ext) < 0) {
            ...
            try {
                ...
                return res.status(404).json({ ok: false, message: 'No hay registros con este id: ' + productID });
            }
            try {
                let producto = product.id;
                GalleryImages = await galleryRepo.createQueryBuilder('galeria')
            ...
            if (GalleryImages < 3) {
                ...
                try {
                    ...
                    result = await cloudinary.v2.uploader.upload(pathImg, { folder: 'gallery' });
                }
                catch (e) {
                    await fs.unlinkSync(pathImg);
                    return res.status(404).json({ ok: false, message: 'Ocurrio un error al intertar subir la imagen a la nube!' });
                }
                //try to save product
                try {
                ...
                return res.json({ ok: false, message: "Numero de imagenes en galeria por producto llego al limite!" });
            }
        }
...
GaleriaController.MostrarGaleria = async (req, res) => {
    ...
    try {
        ...
        return res.json({ ok: false, message: `No se encontro resultado con id: ${productID}` });
    }
    try {
        let producto = product.id;
        GalleryImages = await galleryRepo.createQueryBuilder('galeria')
            ...
            .where({ producto })
            .getMany();
        GalleryImages.length > 0 ? res.json({ ok: true, GalleryImages }) : res.json({ ok: false, message: "Aun no hay galeria para este producto!" });
    }
    catch (error) {
...
GaleriaController.RemoverImagenGaleria = async (req, res) => {
    ...
    try {
        Gallery = await GalleryRepo.findOneOrFail({ where: { id: galleryID } });
    }
    catch (error) {
        return res.json({ ok: false, message: `Galeria con el id: ${galleryID} no encontrada!` });
    }
    ...
    if (req.files === undefined || req.files.foto === undefined) {
        ...
        if (extFile.indexOf(ext) < 0) {
            ...
            try {
                ...
                return res.status(404).json({ ok: false, message: 'No hay registros con este id: ' + galleryID });
            }
            //cambiar nombre del archivo
            ...
            try {
                let id = Gallery.id;
                await galleryRepo.createQueryBuilder().update(Galeria_1.Galeria).set({ imagen: result.secure_url, public_id: result.public_id }).where({ id }).execute();
                await fs.unlinkSync(pathImg);
            }
            catch (error) {
                res.status(409).json({ ok: false, message: 'Algo ha salido mal!' });
