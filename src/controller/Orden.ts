				...
				try {
					...
					if (ordenDetalle.length > 0) {
						...
						for (
							...
							try {
								let qtyExits =
									productoItem.catidad_por_unidad -
									item.cantidad;
								productoItem.catidad_por_unidad = qtyExits;
								const producto = await proRepo.save(
									productoItem
								);
		...
		try {
			...
			const dtsOrdenes = DetoOrdenes.map(async (pro) => {
				...
				if (Product.status == false) {
					let returnProductStock =
						pro.cantidad + Product.catidad_por_unidad;
