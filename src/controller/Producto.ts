...
class ProductoController {
  ...
  static ProductosPaginados = async (req: Request, res: Response) => {
    ...
    let price = Number(req.query.price) || 10000;
    if (typeOrder === 0) {
      order = "ASC";
    } else if (typeOrder === 1) {
      order = "DESC";
  ...
  static ObtenerProductoPorID = async (req: Request, res: Response) => {
    ...
    try {
      ...
      if (!producto) {
        res
          .status(400)
          .json({ ok: false, message: "Error al procesar la peticion" });
        return;
      }
      ...
      return res
        .status(404)
        .json({ ok: false, message: "No hay registros con este id: " + id });
    }
  };
        ...
        if (producto.catidad_por_unidad != 0) {
          return res
            ...
            .json({
              ok: false,
              message:
                "No se puede eliminar un producto con articulos en stock!",
            });
        }
        //producto.status = false
      ...
      if (extFile.indexOf(ext) < 0) {
        ...
        try {
          await productRepo
            .createQueryBuilder()
            .update(Producto)
            .set({ image: result.secure_url, public_id: result.public_id })
            .where({ id })
            .execute();
      ...
      try {
        ...
        if (!producto) {
          ...
          if (producto.catidad_por_unidad == 0) {
            ...
            res
              .status(400)
              .json({ ok: false, message: "Aun hay producto en stock" });
          }
        }
      } catch (error) {
        console.log(error);
