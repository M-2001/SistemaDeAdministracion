		...
		try {
			...
			if (producto.length > 0) {
				...
				res.json({
					ok: false,
					message:
						"No se encontraron resultados con categoria: " +
						categoria,
				});
			}
		} catch (error) {
	...
	static AgregarProducto = async (req: Request, res: Response) => {
		...
		if (codeProductExist) {
			return res.status(400).json({
				ok: false,
				message:
					"Ya existe un producto con el codigo " + codigo_producto,
			});
		}
	...
	static ProductosMasVendidos = async (req: Request, res: Response) => {
		...
		try {
			...
			const formated = productos.map(async (pro) => {
				...
				let totalVenta = DO.map((a) => a.cantidad).reduce(
					(a, b) => a + b,
					0
				);
